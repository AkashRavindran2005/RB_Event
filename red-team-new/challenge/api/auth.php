...
{
    $parts = explode('.', $token);
    // VULNERABILITY: Accept tokens with 2 or 3 parts (allows missing signature)
    ...
    $base64Signature = isset($parts[2]) ? $parts[2] : '';
...
switch ($action) {
        ...
        if (preg_match('/Bearer\s+(.+)/', $authHeader, $matches)) {
            ...
            if ($payload) {
                ...
                if (isset($payload['role']) && $payload['role'] === 'admin') {
                    // Flag only accessible via secrets endpoint
                    if (isset($_GET['secrets']) && $_GET['secrets'] === 'true') {
                        echo json_encode([
                            'success' => true,
                            'classification' => 'TOP SECRET',
                            'flag' => 'CCEE{jwt_4dm1n_s3cr3ts_unl0ck3d}',
                            'message' => 'You have accessed the admin secrets vault via JWT manipulation!'
                        ]);
                    } else {
                        echo json_encode([
                            'success' => true,
                            ...
                            'data' => $payload,
                            'admin_endpoints' => [
                                '/api/auth.php?action=profile&secrets=true' => 'Access classified data'
                            ],
                            'hint' => 'Admin access confirmed. Explore available endpoints.'
                        ]);
                    }
                } else {
                    echo json_encode([
    ...
    case 'debug':
        // Hidden debug endpoint - flag in headers!
        header('X-Internal-Flag: CCEE{h34d3r5_t3ll_s3cr3ts}');
        header('X-Debug-Mode: enabled');
        header('X-Server-Version: CyberTech-API-v1.0.3-beta');
        echo json_encode([
            'status' => 'debug_mode_active',
            'server_time' => date('Y-m-d H:i:s'),
            'hint' => 'Debug information may be exposed in response headers...'
        ]);
        break;
